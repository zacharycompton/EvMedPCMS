...
resultsWhite <- read.csv("pglsLambdaSimResultsWhite50123.csv")
resultsPagel <- read.csv("./pglsLambdaSimResultsPagel50123.csv")
...
ggplot(filtered_rates_with_gaps, aes(x = as.factor(min), y = percent, fill = evo_model)) +
  ...
  labs(
    title = "Error Rates by Evolutionary Model for Each Statistical Model 50 Species",
