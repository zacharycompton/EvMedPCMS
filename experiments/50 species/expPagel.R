...
csv_file_path <- "./pglsLambdaSimResultsPagel50.csv"
...
run_simulation <- function(params) {
  ...
  # Draw random slope between -2 and 2, but avoid values close to zero
  repeat {
    slope <- runif(1, min = -2, max = 2)
    if (abs(slope) > 0.5) break  # Only accept slope if it's not near zero
  }
  ...
  for(sim in 1:num_simulations) {
    ...
    while (!success) {
      success <- tryCatch({
        ...
        maxNeo1 <- max(mamData$RecordsWithDenominators)*maxNeoPrev
        maxNeo2<- max(mamData$NeoplasiaWithDenominators)
        maxNeo <- min(maxNeo1, maxNeo2)
        ...
        mamData$RecordsWithDenominators
        # hist(filter(mamData, RecordsWithDenominators >20)$RecordsWithDenominators)
        # hist(simulated_necropsies)
        ...
        #fitContinuous(pruned.tree, x)
        
        
        # Define lambda for Pagel's lambda model
        lambda <- 0.5  # This specifies partial phylogenetic signal
        
        # Rescale the tree according to Pagel's lambda model
        # This will adjust the tree so that internal branch lengths are scaled by lambda
        rescaled_tree <- rescale(pruned.tree, model = "lambda", lambda = lambda)
        
        # Now simulate 'x' under Brownian motion on the rescaled tree
        x <- fastBM(rescaled_tree, sig2 = var(adult_weight), mu = mean(adult_weight))
        
        fitContinuous(rescaled_tree, x, model = "lambda")
        # Now simulate 'simNeoB' using the Pagel's lambda model with a relationship to 'x'
        if (abs(slope - 0) < 1e-6) {
          # Generate random noise and weaken BM influence to minimize correlation with 'x'
          simNeoB <- fastBM(rescaled_tree, sig2 = var(adult_weight))
        } else {
          # Normal case where slope is non-zero: simulate with noise on the rescaled tree
          noise_level <- sd(adult_weightFrac) * 0.0001  # Define noise level based on fraction of adult weight
          intercept <- 0.05  # Set intercept for the relationship
          
          # Simulate 'simNeoB' with relationship to 'x', adding noise
          simNeoB <- intercept + slope * x + 
            fastBM(rescaled_tree, sig2 = var(adult_weight))   # BM on rescaled tree
          
        }
        
        #fitContinuous(pruned.tree, simNeoB, model = "lambda")
        
        
        # Normalize simNeo values
        simNeo_normalized <- (simNeoB - min(simNeoB)) / (max(simNeoB) - min(simNeoB))
        #Add Gaussian noise after normalization
        noise_level_normalized <- 0.001   # Adjust noise level if needed
        noise <- rnorm(nrow(cutData), mean = 0, sd = noise_level_normalized)*0.1
        simNeo_normalized_noisy <- simNeo_normalized + noise
        
        # Make sure the noisy values still fall within the [0, 1] range after adding noise
        simNeo_normalized_noisy <- pmax(0, pmin(1, simNeo_normalized_noisy))
        
        # Store the values back in the dataframe
        cutData$x <- x
    ...
    tryCatch({
      ...
      adult.weight.neoOptim<-withTimeout({pgls.SEy.Optim(simNeo~logx,data=cutData,tree=pruned.tree,se=SE,model = "lambda")}, timeout = 500, onTimeout = "error")
...
# # Step 1: Configure Gmail API client
# gm_auth_configure(path = "./client_secret_49824980929-ujduoggo0i5kc6p3cmb5hti6oujm5f9n.apps.googleusercontent.com.json")
# 
# # Step 2: Authenticate
...
#   from("your_gmail@example.com") %>%
#   subject("Script Completed") %>%
#   text_body("Your R script has finished running successfully.")
# 
# send_message(email)
# 
write.csv(final_results, file = "./pglsLambdaSimResultsfinalPagel50.csv")
