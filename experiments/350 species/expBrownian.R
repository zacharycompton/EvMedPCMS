...
csv_file_path <- "./pglsLambdaSimResultsBrown350n.csv"
        ...
        for(sim in 1:num_simulations) {
          ...
          while (!success) {
            success <- tryCatch({
              ...
              x <- fastBM(pruned.tree, mu = mean(adult_weight), sig2 = var(adult_weight))
             
              if (abs(slope - 0) < 1e-6) {
                # Generate random noise and weaken BM influence to minimize correlation with x
                simNeoB <- fastBM(pruned.tree)
              } else {
                # Normal case where slope is non-zero, add a relationship between simNeoB and x
                noise_level <- sd(adult_weightFrac) * 0.25
                intercept <- 0.5
                simNeoB <- intercept + slope * x + fastBM(pruned.tree) + rnorm(nrow(cutData), mean = 0, sd = noise_level)
              }
              ...
              noise_level_normalized <- 0.25  # Adjust noise level if needed
              noise <- rnorm(nrow(cutData), mean = 0, sd = noise_level_normalized)
              ...
              #plot(cutData$simNeo ~ log(x), main = "Simulated Neoplasia (simNeo) vs x")
              # summary(pglsSEyPagel(simNeo~log10(x),data=cutData,tree=pruned.tree,se=SE,method = "ML"))
          ...
          tryCatch({
            ...
            #cat("attempting pgls")
            adult.weight.neo<-withTimeout({pglsSEyPagel(simNeo~log10(x),data=cutData,tree=pruned.tree,se=SE,method = "ML")}, timeout = 500, onTimeout = "error")
          ...
          tryCatch({
            ...
            adult.weight.neoOptim<-withTimeout({pgls.SEy.Optim(simNeo~log10(x),data=cutData,tree=pruned.tree,se=SE,model = "BM")}, timeout = 500, onTimeout = "error")
          ...
          tryCatch({
            ...
            pruned.tree$edge.length[pruned.tree$edge.length < min_length] <- min_length
            #cat("attempting gee")
            compar <- withTimeout({suppressMessages(compar.gee(cbind(NeoplasiaOccurences, NonOccurences) ~ log(x), phy = pruned.tree, family = "binomial"))}, timeout = 60, onTimeout = "error")
...
write.csv(final_results, file = "./pglsLambdaSimResultsfinalBrown350.csv")
