...
csv_file_path <- "./pglsLambdaSimResultsPagel350.csv"
  ...
  for(sim in 1:num_simulations) {
    ...
    while (!success) {
      success <- tryCatch({
        ...
        noise_level_normalized <- 0.0001   # Adjust noise level if needed
        noise <- rnorm(nrow(cutData), mean = 0, sd = noise_level_normalized)*0.3
        ...
        cutData$simNeo <- simNeo_normalized_noisy
        cutData<-na.omit(cutData)
        logx<-cutData$logx
...
# send_message(email)
# 
 write.csv(final_results, file = "./pglsLambdaSimResultsfinalPagel350.csv")
