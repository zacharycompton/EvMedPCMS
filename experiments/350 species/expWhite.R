...
csv_file_path <- "./pglsLambdaSimResultsWhite350123.csv"
...
run_simulation <- function(params) {
  ...
  get_best_fit_model_logLik <- function(tree, neo) {
    ...
    # Define the models to test
    models_to_test <- c("BM", "lambda", "OU", "EB")
  ...
  for(sim in 1:num_simulations) {
    ...
    while (!success) {
      success <- tryCatch({
        ...
        x<-runif(nrow(cutData), min = min(adult_weight), max = max(adult_weight))
        cor_threshold <- 0.1  # Define an acceptable correlation threshold, close to 0 for independence
        repeat {
            simNeo <- runif(nrow(cutData), min = 0, max = 1) + runif(nrow(cutData), min = -0.05, max = 0.05)
            simNeo <- pmax(pmin(simNeo, 1), 0)  # Ensure values stay within [0, 1]
            
            # Check correlation between x and simNeo
            cor_value <- cor(x, simNeo)
            if (abs(cor_value) < cor_threshold) {
              break  # Stop if correlation is low enough to consider the variables independent
            }
          }
        ...
        cutData$simNeoComp<-simNeoComp
        simNeo <- asin(sqrt(simNeo))
        cutData$x<-x
      ...
      #plot(cutData$simNeo ~ cutData$x , main = "Simulated Neoplasia (simNeo) vs x")
    ...
    tryCatch({
      ...
      #cat("attempting pgls")
      adult.weight.neo<-withTimeout({pglsSEyPagel(simNeo~x,data=cutData,tree=pruned.tree,se=SE,method = "ML")}, timeout = 500, onTimeout = "error")
    ...
    tryCatch({
      ...
      adult.weight.neoOptim<-withTimeout({pgls.SEy.Optim(simNeo~x,data=cutData,tree=pruned.tree,se=SE,model = best_model)}, timeout = 500, onTimeout = "error")
    ...
    expected_neoplasia_occurrences <- cutData$simNeoComp * cutData$SimulatedNecropsies
    ...
    tryCatch({
      ...
      pruned.tree$edge.length[pruned.tree$edge.length < min_length] <- min_length
      #cat("attempting gee")
      compar <- withTimeout({suppressMessages(compar.gee(cbind(NeoplasiaOccurences, NonOccurences) ~ x, phy = pruned.tree, family = "binomial"))}, timeout = 60, onTimeout = "error")
...
write.csv(final_results, file = "./pglsLambdaSimResultsfinalWhite350123.csv")
