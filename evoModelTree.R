...
name.check(pruned.tree, x)
  ...
  fitBM_sim <- fitContinuous(pruned.tree, x, model = "BM")
  fitOU_sim <- fitContinuous(pruned.tree, x, model = "OU")
  fitLambda<-fitContinuous(pruned.tree, x, model = "lambda")
  fitRateTrend_sim <- fitContinuous(pruned.tree, x, model = "rate_trend")
  fitMeanTrend_sim <- fitContinuous(pruned.tree, x, model = "mean_trend")
  fitWhite_sim <- fitContinuous(pruned.tree, x, model = "white")
  
  testResultsAll <- rbind(
    data.frame(BestModel = "EB", LogLik = fitEB_sim$opt$lnL, nSpecies = length(x)),
    data.frame( BestModel = "BM", LogLik = fitBM_sim$opt$lnL, nSpecies = length(x)),
    data.frame( BestModel = "OU", LogLik = fitOU_sim$opt$lnL, nSpecies = length(x)),
    data.frame( BestModel = "rate_trend", LogLik = fitRateTrend_sim$opt$lnL, nSpecies = length(x)),
    data.frame(BestModel = "mean_trend", LogLik = fitMeanTrend_sim$opt$lnL, nSpecies = length(x)),
    data.frame(BestModel = "white", LogLik = fitWhite_sim$opt$lnL, nSpecies = length(x)),
    data.frame(BestModel = "lambda", LogLik = fitLambda$opt$lnL, nSpecies = length(x))
