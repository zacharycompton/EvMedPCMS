...
modPgls.SEy = function (model, data, corClass = corMartins, tree, se = NULL, 
...
pglsSEyPagelToOptimizeLambda=function(lambda,model,data,tree,...) {
  -logLik(modPgls.SEy(model=model,data=data,tree=tree,corClassValue=lambda,corClass=corMartins,fixed=T,...)) #Returns -logLikelihood of the pgls.SEy model with lambda fixed to the value of the lambda argument. sig2e will be optimized within modPgls.SEy unless given as an argument here
}
...
pglsSEyPagel=function(model, data, tree, lambdaInterval=c(0,1),...){
  ...
  return(modPgls.SEy(model=model,data=data,tree=tree,corClass=corMartins,fixed=T,corClassValue=optimizedModel$minimum,...)) #Returns the final model fit
}
