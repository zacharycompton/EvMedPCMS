...
testResultsAll <- rbind(
  data.frame(BestModel = "EB", LogLik = fitEB_simAll$opt$aic, nSpecies = length(x)),
  data.frame( BestModel = "BM", LogLik = fitBM_simAll$opt$aic, nSpecies = length(x)),
  data.frame( BestModel = "OU", LogLik = fitOU_simAll$opt$aic, nSpecies = length(x)),
  data.frame( BestModel = "rate_trend", LogLik = fitRateTrend_simAll$opt$aic, nSpecies = length(x)),
  data.frame(BestModel = "mean_trend", LogLik = fitMeanTrend_simAll$opt$aic, nSpecies = length(x)),
  data.frame(BestModel = "white", LogLik = fitWhite_simAll$opt$aic, nSpecies = length(x)),
  data.frame(BestModel = "lambda", LogLik = fitLambdaAll$opt$aic, nSpecies = length(x)),
  data.frame(BestModel = "kappa", LogLik = fitKappaAll$opt$aic, nSpecies = length(x)),
  data.frame(BestModel = "delta", LogLik = fitDeltaAll$opt$aic, nSpecies = length(x))
  
)
max_row <- testResultsAll[which.min(testResultsAll$LogLik), ]
resultsAll<-rbind(testResultsAll, max_row)


modelsBestFitAll<-na.omit(resultsAll)

sorted_resultsAll <- modelsBestFitAll[order(modelsBestFitAll$LogLik), ]
  ...
  tryCatch({
    ...
    tryCatch({
      ...
      testResults <- rbind(
        ...
        data.frame(Class = class_name, BestModel = "mean_trend", LogLik = fitMeanTrend_sim$opt$aic, nSpecies = length(x)),
        data.frame(Class = class_name, BestModel = "white", LogLik = fitWhite_sim$opt$aic, nSpecies = length(x)),        
        data.frame(Class = class_name, BestModel = "EB", LogLik = fitLambda_sim$opt$aic, nSpecies = length(x)),
        data.frame(Class = class_name, BestModel = "lambda", LogLik = fitLambda_sim$opt$aic, nSpecies = length(x)),
        data.frame(Class = class_name, BestModel = "kappa", LogLik = fitKappa_sim$opt$aic, nSpecies = length(x)),
        data.frame(Class = class_name, BestModel = "delta", LogLik = fitDelta_sim$opt$aic, nSpecies = length(x))
...
for (class_name in OrderNames) {
  ...
  orderClass<-as.character(unique(filterData$Class))
  bestClassModelRow<-filter(modelsBestFitClass, Class == orderClass)
  bestClassModel<-as.character(bestClassModelRow$BestModel)
  #for malignancy
  cutData <- filterData[, c(9,17), drop=FALSE]
  # for neo
  #cutData <- filterData[, c(9,13), drop=FALSE]
  ...
  results <- tryCatch({
    ...
    testResults <- rbind(
      data.frame(Orders = class_name, BestModel = "EB", LogLik = fitEB_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "BM", LogLik = fitBM_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "OU", LogLik = fitOU_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "rate_trend", LogLik = fitRateTrend_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "mean_trend", LogLik = fitMeanTrend_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "white", LogLik = fitWhite_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "lambda", LogLik = fitLambda_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "kappa", LogLik = fitKappa_sim$opt$aic, nSpecies = length(x)),
      data.frame(Orders = class_name, BestModel = "delta", LogLik = fitDelta_sim$opt$aic, nSpecies = length(x))
    )
    sorted_results <- testResults[order(testResults$LogLik), ]
    ...
    if ((sorted_results$LogLik[3] - sorted_results$LogLik[1]) <= 3) {
      # If "lambda" is among the top three models, select it as the best model
      if (bestClassModel %in% sorted_results$BestModel[1:3]) {
        max_row <- sorted_results[sorted_results$BestModel == "lambda", ][1, ]
      } else {
        # If "lambda" is not among the top three, select the model with the lowest AIC
        max_row <- sorted_results[1, ]
